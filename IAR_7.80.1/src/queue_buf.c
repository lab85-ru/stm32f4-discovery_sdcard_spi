...
int push_data_queue( queue_buf_t *q )
{
    ...
    // test mesto est?
    if ( get_free_size_queue( q ) < q->w_len ){                    // Error No Memory to queue for Write
        return 1;
    }

    for (i=0; i<q->w_len; i++){
        *(q->queue + q->in) = *(q->w_buf + i);
        (q->in)++;
        if ( q->in >= q->len ){
            q->in = 0;
        }
...
int pop_data_queue( queue_buf_t *q )
{
    ...
    // test mesto est?
    if ( (q->len - get_free_size_queue( q )) < q->r_len ){         // Error No pop data
        return 1;
    }

    for (i=0; i< q->r_len; i++){
        *(q->r_buf + i) = *(q->queue + q->out);
        (q->out)++;
